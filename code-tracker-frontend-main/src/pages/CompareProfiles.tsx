...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-black/80 p-2 border border-blue-400/50 rounded shadow text-blue-100">
      ...
      <div className="mt-8 space-y-8">
        <div className="grid grid-cols-2 gap-8">
          {comparisonData.map(data => (
            <div key={data.handle} className="p-6 bg-black/60 rounded-lg shadow-lg border border-blue-400/30">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-black/40 rounded border border-blue-400/20">
                  <p className="text-sm text-blue-200">Current Rating</p>
                ...
                <div className="p-4 bg-black/40 rounded border border-blue-400/20">
                  <p className="text-sm text-blue-200">Max Rating</p>
                ...
                <div className="p-4 bg-black/40 rounded border border-blue-400/20">
                  <p className="text-sm text-blue-200">Best Rank</p>
        ...
        <div className="bg-black/60 p-6 rounded-lg shadow-lg border border-blue-400/30">
          <h3 className="text-xl font-bold mb-4 text-blue-300">Contest Rating Progress</h3>
            ...
            >
              ...
              <Tooltip 
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-black/80 p-3 border border-blue-400/50 shadow-lg text-blue-100">
                        <p className="font-bold text-base text-blue-300">{data.contestName}</p>
              ...
              {comparisonData.map((data, index) => (
                <Line
                  ...
                  type="linear"
                  dataKey={data.handle}
                  stroke={index === 0 ? "#4ade80" : "#60a5fa"}
                  strokeWidth={2}
                  dot={{ r: 4, fill: index === 0 ? "#4ade80" : "#60a5fa", strokeWidth: 0 }}
                  activeDot={{ r: 6, stroke: index === 0 ? "#4ade80" : "#60a5fa", strokeWidth: 0 }}
                  connectNulls
        ...
        <div className="bg-black/60 p-6 rounded-lg shadow-lg border border-blue-400/30">
          <h3 className="text-xl font-bold mb-4 text-blue-300">Problem Ratings Comparison</h3>
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={ratingData}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#ffffff" />
            <XAxis dataKey="rating" tick={{ fill: '#ffffff' }} stroke="#ffffff" />
            <YAxis tick={{ fill: '#ffffff' }} stroke="#ffffff" />
            <Tooltip />
            <Legend />
                            <Bar dataKey={comparisonData[0].handle} fill="#4ade80" />
              <Bar dataKey={comparisonData[1].handle} fill="#60a5fa" />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-black/60 p-6 rounded-lg shadow-lg border border-blue-400/30">
          <h3 className="text-xl font-bold mb-4 text-blue-300">Problem Tags Comparison</h3>
          <ResponsiveContainer width="100%" height={600}>
            <BarChart data={tagData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff" />
              <XAxis type="number" tick={{ fill: '#ffffff' }} stroke="#ffffff" />
              <YAxis type="category" dataKey="tag" width={150} tick={{ fill: '#ffffff' }} stroke="#ffffff" />
              <Tooltip />
              <Legend />
              <Bar dataKey={comparisonData[0].handle} fill="#4ade80" />
              <Bar dataKey={comparisonData[1].handle} fill="#60a5fa" />
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 flex items-center justify-center px-4">
        <div className="w-full max-w-4xl">
          <h1 className="text-3xl font-bold mb-8 text-blue-300 text-center">Compare Profiles</h1>
          ...
          <div className="flex space-x-4 mb-8">
            <input
              ...
              className="flex-1 p-2 border border-blue-400/50 rounded bg-black/40 text-blue-100 placeholder-blue-300/70 focus:outline-none focus:ring-cyan-400 focus:border-cyan-400"
            />
            <input
              type="text"
              value={handle2}
            ...
            <button
              ...
              disabled={loading}
              className="px-6 py-2 bg-cyan-600 text-white rounded hover:bg-cyan-700 disabled:bg-cyan-300"
            >
          ...
          {error && (
            <div className="p-4 mb-8 bg-red-900/50 text-red-300 rounded border border-red-400/30">
              {error}
            </div>
          )}
        </div>
